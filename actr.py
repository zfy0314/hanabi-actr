...
class actr:
    ...
    def add_command(
        ...
        if name in self.interface.commands.keys():
            if self.interface.commands[name] == function:
                ...
                print(
                    "Command ",
                    name,
                    " already exists and is now being replaced by ",
                    function,
                )
        ...
        if existing[0] == True:
            if existing[1] == None:
                result = self.interface.send(
                    "add", name, call_name, documentation, single, actr_name, encoded
                )
                if result[0]:
                    self.interface.add_command(name, function)
                    return result[1]
                ...
                print(
                    "Cannot add command ",
                    name,
                    " because it has already been added by a different owner.",
                )
                return False
            else:
                return True
        ...
        if port == None and os.path.isfile(portfile):
            with open(portfile, "r") as f:
                try:
                    ...
                    print(
                        "Problem reading ACT-R port number from",
                        portfile,
                        ". Using default or 2650.",
                    )
                    port = 2650
        elif port == None:
            print(
                "ACT-R port number file", portfile, "not found. Using default or 2650."
            )
            port = 2650
        ...
        if host == None and os.path.isfile(hostfile):
            with open(hostfile, "r") as f:
                try:
                    ...
                    print(
                        "Problem reading ACT-R host from",
                        hostfile,
                        ". Using default of 127.0.0.1.",
                    )
                    host = "127.0.0.1"
        elif host == None:
...
class interface:
    def __init__(self, host, port):
        ...
        try:
            ...
            print(
                "Error trying to connect to ACT-R at",
                host,
                ":",
                port,
                "with exception",
        ...
        if not (self.echo):
            ...
            while not (ready):
                ...
                if existing[1] == None:
                    self.send(
                        "add",
                        "python-echo" + str(self.echo_count),
                        "echo",
                        "Trace monitor for python client.  Do not call directly.",
                        True,
                    )
                    ready = True
                else:
    ...
    def no_output(self):
        ...
        if self.echo:
            self.send(
                "remove-monitor", "model-trace", "python-echo" + str(self.echo_count)
            )
            self.send(
                "remove-monitor", "command-trace", "python-echo" + str(self.echo_count)
            )
            self.send(
                "remove-monitor", "warning-trace", "python-echo" + str(self.echo_count)
            )
            self.send(
                "remove-monitor", "general-trace", "python-echo" + str(self.echo_count)
            )
...
def set_current_model(name):
    if name.lower() in (x.lower() for x in mp_models()):
        ...
        print(
            "%s is not one of the currently available models: %s" % (name, mp_models())
        )
...
def open_exp_window(title, visible=True, width=300, height=300, x=300, y=300):
    return current_connection.evaluate_single(
        "open-exp-window",
        title,
        [
            ["visible", visible],
...
def add_text_to_exp_window(
    window, text, x=0, y=0, color="black", height=20, width=75, font_size=12
):
    return current_connection.evaluate_single(
...
def add_button_to_exp_window(
    window, text="", x=0, y=0, action=None, height=20, width=75, color="gray"
):
    return current_connection.evaluate_single(
...
def remove_items_from_exp_window(window, *items):
    return current_connection.evaluate_single(
        "remove-items-from-exp-window", window, *items
    )
...
def add_command(
    ...
    return current_connection.add_command(
        name, function, documentation, single, local_name, encoded
    )
...
def new_tone_sound(freq, duration, onset=False, time_in_ms=False):
    return current_connection.evaluate_single(
        "new-tone-sound", freq, duration, onset, time_in_ms
    )


def new_word_sound(word, onset=False, location="external", time_in_ms=False):
    return current_connection.evaluate_single(
        "new-word-sound", word, onset, location, time_in_ms
    )


def new_digit_sound(digit, onset=False, time_in_ms=False):
    return current_connection.evaluate_single(
        "new-digit-sound", digit, onset, time_in_ms
    )
...
def buffer_slot_value(buffer_name, slot_name):
    return current_connection.evaluate_single(
        "buffer-slot-value", buffer_name, slot_name
    )


def set_chunk_slot_value(chunk_name, slot_name, new_value):
    return current_connection.evaluate_single(
        "set-chunk-slot-value", chunk_name, slot_name, new_value
    )
...
def set_buffer_chunk(buffer_name, chunk_name, requested=True):
    return current_connection.evaluate_single(
        "set-buffer-chunk", buffer_name, chunk_name, requested
    )
...
def add_line_to_exp_window(window, start, end, color=False):
    if color:
        return current_connection.evaluate_single(
            "add-line-to-exp-window", window, start, end, color
        )
    else:
        return current_connection.evaluate_single(
            "add-line-to-exp-window", window, start, end
        )
...
def modify_line_for_exp_window(line, start, end, color=False):
    if color:
        return current_connection.evaluate_single(
            "modify-line-for-exp-window", line, start, end, color
        )
    else:
        return current_connection.evaluate_single(
            "modify-line-for-exp-window", line, start, end
        )
...
def schedule_event_after_module(
    after_module,
    action,
    params=None,
    module=":NONE",
    ...
    return current_connection.evaluate(
        ...
        [
            ...
            ["dynamic", dynamic],
            ["precondition", precondition],
            ["include-maintenance", include_maintenance],
        ],
    )


def schedule_break_relative(
    time_delay, time_in_ms=False, priority=":max", details=None
):
    return current_connection.evaluate_single(
        "schedule-break-relative",
        time_delay,
        [["time-in-ms", time_in_ms], ["priority", priority], ["details", details]],
    )
...
def history_data_available(history, file=False, *params):
    return current_connection.evaluate_single(
        "history-data-available", history, file, *params
    )


def process_history_data(
    processor, file=False, data_params=None, processor_params=None
):
    return current_connection.evaluate_single(
        "process-history-data", processor, file, data_params, processor_params
    )


def save_history_data(history, file, comment="", *params):
    return current_connection.evaluate_single(
        "save-history-data", history, file, comment, *params
    )
...
def print_chunk_activation_trace(chunk, time, ms=True):
    return current_connection.evaluate_single(
        "print-chunk-activation-trace", chunk, time, ms
    )
...
def schedule_simple_set_buffer_chunk(
    buffer, chunk, time, module="NONE", priority=0, requested=True
):
    return current_connection.evaluate_single(
        "schedule-simple-set-buffer-chunk",
        buffer,
        chunk,
        time,
        module,
...
def schedule_simple_mod_buffer_chunk(
    buffer, mod_list_or_spec, time, module="NONE", priority=0
):
    return current_connection.evaluate_single(
        "schedule-simple-mod-buffer-chunk",
        buffer,
        mod_list_or_spec,
        time,
        module,
...
def define_module(name, buffers, params, interface=None):
    return current_connection.evaluate_single(
        "define-module", name, buffers, params, interface
    )
...
def create_image_for_exp_window(
    window, text, file, x=0, y=0, width=50, height=50, action=None
):
    return current_connection.evaluate_single(
        "create-image-for-exp-window",
        window,
        text,
        file,
...
def add_image_to_exp_window(
    window, text, file, x=0, y=0, width=50, height=50, action=None
):
    return current_connection.evaluate_single(
        "add-image-to-exp-window",
        window,
        text,
        file,
        [["x", x], ["y", y], ["width", width], ["height", height], ["action", action]],
...
add_command(
    ...
    env_loader,
    "Import a Python module and make it available directly from the interactive prompt. Params: pathname",
)

add_command(
    "load-python-module-html",
    env_loader_no_path,
    "Import a python module from the directory containing the actr.py module and make it available directly from the interactive prompt. Params: filename",
)
